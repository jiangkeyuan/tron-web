const detail = {
  q1: {
    ...
    content: `
        ...
        <p>波场账户能量不是免费的,需要冻结TRX获取能量。也可以向HashGo能量交易平台租能量。假如账户中有100万能量,使用完毕后,会慢慢恢复,24小时恢复满100万。冻结(租用)默认最少3天, 当3天后账户取消冻结(租用到期)时,通过冻结(租用)获得的能量也将自动清零。</p>
        `
  },
  q2: {
    title: '能量有什么用？',
  ...
  q3: {
    ...
    <p>方法三:租用能量</p>
    <p>可以通过HashGo平台进行能量租赁的方式来节省手续费,能量租赁既不需要冻结大量TRX,也不用燃烧更多TRX,还可节省至少80%的手续费,以租用100万能量为例,若直接燃烧TRX,3天需要燃烧大约840TRX,用HashGo平台租用100万能量只需要140TRX左右,节省了83.33%≈700TRX。推荐使用,具体可参考《如何租用能量？》</p>
        `
  },
  q4: {
    title: '如何租用能量？',
    ...
    <p>HashGo提供了波场能量交易功能,用户可随时自助租用,实时到账。租用流程简单便捷,可选择DAPP租用和转账租用两种方式:</p>
    <p>DAPP租用:</p>
    <p>1.打开租赁网址HashGo,点击登录连接DAPP钱包</p>
    ...
    <p>1.打开租赁网址HashGo,输入想要租用的能量和租用天数,系统自动计算出需要转账的金额</p>
